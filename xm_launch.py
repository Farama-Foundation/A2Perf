...
_EXPERIMENT_NAME = flags.DEFINE_string("experiment-name", None,
                                       "Experiment name")
...
def main(_):
    ...
    with create_experiment(
        ...
        async def make_job(work_unit: xm.WorkUnit,
            ...
            docker_gin_config_path = os.path.join(full_root_dir,
                                                  GENERIC_GIN_CONFIG_NAME)
            try:
                shutil.copy(_SUBMISSION_GIN_CONFIG_PATH.value,
                            docker_gin_config_path)
            except IOError as e:
            ...
            base_image = _GPU_BASE_IMAGE.value if _NUM_GPUS.value > 0 else _CPU_BASE_IMAGE.value
            ...
            [executable] = experiment.package(
                    [
                            xm.python_container(
                                    ...
                                    entrypoint=get_entrypoint(_DOMAIN.value,
                                                              _USER.value),
                            )
                    ]
            )
            ...
            hparams.update({
                    ...
                    "gin-config": os.path.join(DOCKER_EXPERIMENT_DIR,
                                               GENERIC_GIN_CONFIG_NAME),
