...
_MODE = flags.DEFINE_enum(
    "mode", "train", ["train", "inference", "generalization"], "Mode of execution"
)
_JOB_TYPE = flags.DEFINE_enum(
    "job_type",
    None,
    ["train", "collect", "inference", "reverb", "generalization"],
    "Type of job",
)
...
_SKILL_LEVELS = flags.DEFINE_list("skill_levels", ["novice"], "Skill levels to run")
...
def get_web_navigation_hparam_sweeps(**kwargs):
    ...
    for task_name, hparams in task_hyperparameters.items():
        if task_name in task_names:
            for params in itertools.product(*hparams.values()):
                task_sweeps.append(dict(zip(hparams.keys(), params)))
    ...
    algo_hyperparameters = {
        "bc": {
            ...
            "learning_rate": [1e-4],
            "env_batch_size": [1],
            # still need this to init locomotion env
            "num_epochs": [0],
            "num_iterations": [5000],
    ...
    for algo, hparams in algo_hyperparameters.items():
        if algo in algos:
            for params in itertools.product(*hparams.values()):
                algo_sweeps.append(dict(zip(hparams.keys(), params)))
...
def get_circuit_training_hparam_sweeps(**kwargs):
    ...
    task_hyperparameters = {
        "netlist_toy_macro_stdcell_std_cell_placer_mode_dreamplace": {
            "task_name": ["netlist_toy_macro_stdcell_std_cell_placer_mode_dreamplace"],
            "netlist_path": [
                os.path.join(
...
def main(_):
    ...
    with create_experiment(experiment_title=_EXPERIMENT_NAME.value) as experiment:
        ...
        async def make_job(work_unit: xm.WorkUnit, **hparams):
            ...
            hparams["max_sequence_length"] = TASK_TO_MAX_SEQUENCE_LENGTH[_DOMAIN.value][
                task
            ]
