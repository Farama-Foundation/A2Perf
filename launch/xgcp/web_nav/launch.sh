...
#sudo mkdir -p -m 0777 "$ROOT_DIR/logs/"
#sudo mkdir -p -m 0777 "$ROOT_DIR/logs/collect_logs"
...
# if statement for debug mode for cuda visible devices
#if [ "$DEBUG_MODE" = true ]; then
#  ENV_SETTINGS_BASE=$ENV_SETTINGS_BASE' export NVIDIA_VISIBLE_DEVICES=0 && export CUDA_VISIBLE_DEVICES=0 &&'
#fi
...
$ENV_SETTINGS_BASE export JOB_TYPE='reverb' && \
  export METRIC_VALUES_DIR='$ROOT_DIR/metrics/reverb-server' && \
  export EXTRA_GIN_BINDINGS='--extra_gin_bindings=\"track_emissions.default_cpu_tdp=95\"' && \
  tmux new-session -d -s 'launch_ct' -n 'reverb' 'bash /home/ikechukwuu/workspace/rl-perf/launch/xgcp/circuit_training/launch_worker.sh && zsh' && echo 'Done'
EOF
)
...
ENV_SETTINGS=$(
  cat <<EOF
$ENV_SETTINGS_BASE export JOB_TYPE='train' && \
  export METRIC_VALUES_DIR='$ROOT_DIR/metrics/train-server' && \
  export EXTRA_GIN_BINDINGS='--extra_gin_bindings=\"track_emissions.default_cpu_tdp=125\"' && \
  tmux new-session -d -s 'launch_ct' -n 'train' 'bash /home/ikechukwuu/workspace/rl-perf/launch/xgcp/circuit_training/launch_worker.sh && zsh' && echo 'Done'
EOF
)
echo 'ENV_SETTINGS train: ' "$ENV_SETTINGS"

 Start the reverb server using expect
/usr/bin/expect <<EOD
set timeout 20
spawn ssh ikechukwuu@train-server -i /home/ikechukwuu/.ssh/google_compute_engine $ENV_SETTINGS
expect {
    "Are you sure you want to continue connecting (yes/no)?" {
        send "yes\r"
        exp_continue
...
for ((i = 0; i < NUM_COLLECT_SERVERS; i++)); do
  ...
  ENV_SETTINGS=$(
    cat <<EOF
$ENV_SETTINGS_BASE export JOB_TYPE='collect' && \
  export METRIC_VALUES_DIR='$ROOT_DIR/metrics/$instance_name' && \
  export EXTRA_GIN_BINDINGS='--extra_gin_bindings=\"track_emissions.default_cpu_tdp=125\"' && \
...
EOF
  ...
  /usr/bin/expect <<EOD
set timeout 20
spawn ssh ikechukwuu@$instance_name -i /home/ikechukwuu/.ssh/google_compute_engine $ENV_SETTINGS
expect {
    "Are you sure you want to continue connecting (yes/no)?" {
        send "yes\r"
        exp_continue
