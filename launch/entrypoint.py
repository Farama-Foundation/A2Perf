...
_EPSILON_GREEDY = flags.DEFINE_float("epsilon_greedy", None, "Epsilon greedy value.")
...
_MOTION_FILE_PATH = flags.DEFINE_string("motion_file_path", None, "Motion file path.")
...
_BATCH_SIZE = flags.DEFINE_integer("batch_size", None, "Batch size for training.")
...
_RB_CAPACITY = flags.DEFINE_integer("rb_capacity", None, "Replay buffer capacity.")
...
_NETLIST_PATH = flags.DEFINE_string("netlist_path", None, "Path to the netlist file.")
...
_DEFAULT_CPU_TDP = flags.DEFINE_integer("default_cpu_tdp", 0, "Default CPU TDP.")
...
def main(_):
    ...
    os.environ["TRAIN_CHECKPOINT_INTERVAL"] = str(_TRAIN_CHECKPOINT_INTERVAL.value)
    os.environ["POLICY_CHECKPOINT_INTERVAL"] = str(_POLICY_CHECKPOINT_INTERVAL.value)
    ...
    os.environ["NUM_EPISODES_PER_ITERATION"] = str(_NUM_EPISODES_PER_ITERATION.value)
    ...
    os.environ["REPLAY_BUFFER_SERVER_ADDRESS"] = _REPLAY_BUFFER_SERVER_ADDRESS.value
    os.environ["REPLAY_BUFFER_SERVER_PORT"] = str(_REPLAY_BUFFER_SERVER_PORT.value)
    ...
    if _JOB_TYPE.value == "collect":
        ...
        os.environ["POLICY_NAME"] = _POLICY_NAME.value
        logging.info(
            "Running generalization experiments with the policy %s.", _POLICY_NAME.value
        )
    ...
    if _JOB_TYPE.value == "evaluate":
        ...
        subprocess.run(generate_command, env=os.environ.copy(), text=True, check=True)
        return
