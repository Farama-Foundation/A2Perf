...
_NUM_EPISODES = flags.DEFINE_integer('num_episodes', 100,
...
def perform_rollouts(policy: TFPolicy, env: gym.Env,
...
def collect_dataset(env_name, checkpoint_path, unique_id, num_episodes, seed, ):
  unique_id = f'{unique_id:03d}'
  dataset_path = os.path.join(_DATASETS_PATH.value,
                              str(unique_id))
  ...
  if env_name == 'QuadrupedLocomotion-v0':
    infinite_observation_space = spaces.Box(low=-np.inf, high=np.inf,
                                            shape=env.observation_space.shape,
                                            dtype=env.observation_space.dtype)
    data_collector_env = DataCollector(env,
  ...
  policy = load_policy(saved_model_path=saved_model_path,
                       checkpoint_path=checkpoint_path)
  ...
  temp_dataset_id = f'{_DATASET_ID.value[:-3]}-{unique_id}-v0'
  dataset = data_collector_env.create_dataset(dataset_id=temp_dataset_id, )
  return dataset
...
def main(_):
  ...
  num_episodes_per_policy = np.ceil(
      _NUM_EPISODES.value / len(evaluation_df)).astype(int)
  ...
  dataset = minari.combine_datasets(datasets_to_combine=datasets,
                                    new_dataset_id=_DATASET_ID.value)
