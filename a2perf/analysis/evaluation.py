...
def main(_):
    ...
    saved_model_path = os.path.join(_ROOT_DIR.value, "policies", _POLICY_NAME.value)
    ...
    total_training_energy_kwh = (
        training_system_df.groupby(["domain", "algo", "task", "experiment", "seed"])[
            "energy_consumed"
        ]
        .last()
        .sum()
    ...
    # Save as JSON
    evaluation_save_path = os.path.join(_ROOT_DIR.value, "policies", "evaluation.json")
